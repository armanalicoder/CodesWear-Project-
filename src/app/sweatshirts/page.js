...
async function getData() {
  ...
  try {
    let Sweatshirts = {};
    const products = await Product.find({ category: "Sweatshirts" });
    for (let item of products) {
      if (item.title in Sweatshirts) {
        if (
          !Sweatshirts[item.title].color.includes(item.color) &&
          item.avlQty > 0
        ) {
          Sweatshirts[item.title].color.push(item.color);
        }
        if (
          !Sweatshirts[item.title].size.includes(item.size) &&
          item.avlQty > 0
        ) {
    ...
    console.log("Error During Fetching Sweatshirts", err);
  }
}

async function Sweatshirts() {
  ...
  return (
    <div className="container px-5 py-10 mx-auto">
      ...
      <div className="mx-auto">
        {!products && <p className="text-center mt -2">Out of Stock.</p>}
                  ...
                  <div className="mt-4">
                    ...
                    <div className="space-x-3 mt-2">
                      ...
                      {products[item].size.includes("XXL") && (
                        <span className="border p-1">XXL</span>
                      )}
                    </div>
                    <div className="space-x-3 mt-2">
                      ...
                      {products[item].color.includes("black") && (
                        <button className="border-2 border-gray-300 bg-black ml-1  rounded-full w-6 h-6 focus:outline-none"></button>
                      )}
