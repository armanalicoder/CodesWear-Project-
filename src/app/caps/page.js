...
async function getData() {
  ...
  try {
    let Caps = {};
    const products = await Product.find({ category: "Caps" });
    for (let item of products) {
      if (item.title in Caps) {
        if (!Caps[item.title].color.includes(item.color) && item.avlQty > 0) {
          Caps[item.title].color.push(item.color);
        }
        if (!Caps[item.title].size.includes(item.size) && item.avlQty > 0) {
    ...
    console.log("Error During Fetching Caps", err);
  }
}
