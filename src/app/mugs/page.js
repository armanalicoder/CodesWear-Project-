  ...
  try {
    ...
    for (let item of products) {
      if (item.title in Mugs) {
        if (!Mugs[item.title].color.includes(item.color) && item.avlQty > 0) {
          Mugs[item.title].color.push(item.color);
        }
        if (!Mugs[item.title].size.includes(item.size) && item.avlQty > 0) {
    ...
    console.log("Error During Fetching Mugs", err);
  }
}
                  ...
                  <div className="mt-4">
                    ...
                    <div className="space-x-3 mt-2">
                      {products[item].size.includes("standard") && (
                        <span className="border p-1">STANDARD</span>
                      )}
